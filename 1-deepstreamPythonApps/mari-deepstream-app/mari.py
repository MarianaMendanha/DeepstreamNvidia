...
def main(args):
    ...
    if len(args) != 3:
        ...
        sys.exit(1)
    if args[1] != "-stream" and args[1] != "-save":
        print("ue")
        sys.stderr.write("choose args -stream or -save")
        sys.exit(1)
    ...
    if args[1]=="-stream":
        ...
        capss = Gst.Caps.from_string("video/x-raw, format=I420")
        capsfilter.set_property("caps", capss)
        ...
        if not container:
            sys.stderr.write(" Unable to create code parser \n")
        # Finally save the osd output
        print("Creating FileSink \n")
        sink = Gst.ElementFactory.make("filesink", "filesink")
        if not sink:
            sys.stderr.write(" Unable to create file sink \n")
        sink.set_property("location", "./out_people_mari.mp4")
    ...
    pgie.set_property('config-file-path', "config_infer_primary_mari.txt")
    ...
    Gst.debug_bin_to_dot_file(pipeline, Gst.DebugGraphDetails.ALL, "pipeline_dot")
